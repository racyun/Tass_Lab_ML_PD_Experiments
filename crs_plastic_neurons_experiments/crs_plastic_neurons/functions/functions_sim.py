...
def genBackup_Files( BackupFolderName, kStep, v, s, snoise, VT, switchOffSpikeTimes, cMatrix, synConnections, STNCenter, GPeCenter, lastSpikeTimeStep, evenPriorLastSpikeTimeStep , delayedSpikingNeurons):
    ...
    print(BackupFolderName+'/evenPriorLastSpikeTimeStep.npy')
    ...
    try:
        ...
        print(BackupFolderName+'/delayedSpikingNeurons.npy')
    except:
   ...
   # threshold potentials
    ...
    #CHANGE STARTS HERE
    new_system_state = []
    for element in systemState:
        if element.shape[0]!=1:
            if element.shape==(1002,2):
                element1, element2 = element[:, 0], element[:, 1]
                new_system_state.append(element1)
                new_system_state.append(element2)
            else:
                new_system_state.append(element)
    systemState = tuple(new_system_state)
    # CHANGE ENDS HERE
...
def reproduce_initial_conditions( system_parameters , synConnections, cMatrix ):
   ...
   # integration time step
    ...
    # filter already consideres coupling strengths and synaptic timescales according to differential equations
    # excitatory weights
    basicFilterSpikingNeurons[:N_STN,0]=cMaxExc*np.ones(N_STN) # /tauSynExc
    # inhibitory weights
    basicFilterSpikingNeurons[N_STN:,1]=cMaxInh*np.ones(N_GPe) # /tauSynInh
